...
class QRCodeReader(object):
    ...
    def __call__(self, img, thresh=80, max_val=255):
        ...
        self._ret, *self._data = self.qr.detectAndDecodeMulti(binary)
        ...
        # QRコードの描画
        diff = max_val // len(self._data[0])
        for i, (txt, pts, straight_qrcode) in enumerate(zip(*self._data)):
...
def main(args):
    ...
    while True:
        ...
        dst, _data = qr(frame, thresh).img
        # print(_data[1])

        # 最初に検知した文字列を記録する
        if firstStr==None and _data[0]!=tuple():
            firstStr=_data[0][0]
        #print(firstStr)
        plot = None
        # 最初に検知したqrコードの座標をprintする
        for i,str in enumerate(_data[0]):
            if str!=firstStr: continue
            print(_data[1][i,:,:])
            plot = _data[1][i,:,:]
            break
        ...
        if plot is not None:
            ...
            S1 =  ((plot[3][0]-plot[1][0])*(plot[0][1]-plot[1][1])-(plot[3][1]-plot[1][1])*(plot[0][0]-plot[1][0]))/2
            S2 =  ((plot[3][0]-plot[1][0])*(plot[1][1]-plot[2][1])-(plot[3][1]-plot[1][1])*(plot[1][0]-plot[2][0]))/2
        
            C1_x = plot[0][0] + (plot[2][0]-plot[0][0])*S1/(S1 + S2)
            C1_y = plot[0][1] + (plot[2][1]-plot[0][1])*S1/(S1 + S2)
